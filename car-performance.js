...
function lapShareChart(rows, key, reference) {
  ...
  return `<p class="performance-note">Reference: ${escape(reference||'fastest measured lap')} · time lost per lap, expressed as % of the reference lap. Negative means time gained.</p><div class="performance-bars">${available.map(r=>`<div class="performance-bar-row" title="${escape(r.team)}: ${signed(r[key],3)} of a lap"><span>${teamLabel(r)}</span><div class="performance-bar-track"><i class="performance-zero" style="left:${zero}%"></i><i class="performance-bar" style="left:${(Math.min(0,r[key])-lo)/span*100}%;width:${Math.abs(r[key])/span*100}%;background:${color(r.color)}"></i></div><span>${signed(r[key])}</span></div>`).join('')}</div>`;
}
...
async function analyse() {
  ...
  $('performanceStatus').textContent=`${context.year} · best qualifying lap · ${events.length}/${picked.length} events with data${errors.length ? ` · ${errors.length} data requests unavailable` : ''}. Fresh retrieval complete.`;
  render();
}
