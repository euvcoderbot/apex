      ...
      ${name === 'Speed trace' ? `
        <div class="speed-chart-controls">
          <div class="trace-settings" aria-label="Telemetry display settings">
            ...
            <label class="trace-setting trace-mode-toggle" title="Enhanced reconstructs held or missing telemetry samples and derives timing delta from that same speed curve. Official sector and finish deltas remain exact."><input type="checkbox" id="interpolationToggle" ${enhancedTraceMode ? 'checked' : ''}><i aria-hidden="true"></i><span>Enhanced interpolation</span><small id="traceModeStatus" data-mode="${enhancedTraceMode ? 'enhanced' : 'accurate'}">${enhancedTraceMode ? 'INTERPOLATED' : 'ACCURATE'}</small></label>
